...
public abstract class TaskViewTouchController<T extends BaseDraggingActivity>
    ...
    public void onDragEnd(float velocity, boolean fling) {
        ...
        if (fling) {
            logAction = LauncherLogProto.Action.Touch.FLING;
            boolean goingUp = velocity < 0;
            goingToEnd = goingUp == mCurrentAnimationIsGoingUp;
        } else {
            logAction = LauncherLogProto.Action.Touch.SWIPE;
        ...
        float nextFrameProgress = Utilities.boundToRange(
                progress + velocity * Utilities.SINGLE_FRAME_MS / Math.abs(mEndDisplacement), 0f, 1f);
        ...
        anim.setInterpolator(Interpolators.scrollInterpolatorForVelocity(velocity));
        anim.start();
    }
