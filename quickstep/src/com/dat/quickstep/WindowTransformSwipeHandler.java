...
import static com.dat.quickstep.QuickScrubController.QUICK_SCRUB_FROM_APP_START_DURATION;
...
import com.dat.launcher3.InvariantDeviceProfile;
import com.dat.launcher3.LauncherAppState;
import com.dat.launcher3.R;
import com.dat.launcher3.Utilities;
import com.dat.launcher3.anim.AnimationSuccessListener;
...
import com.dat.quickstep.util.ClipAnimationHelper;
import com.dat.quickstep.util.RemoteAnimationTargetSet;
import com.dat.quickstep.util.TransformedRect;
...
import com.dat.launcher3.BaseActivity;
...
public class WindowTransformSwipeHandler<T extends BaseDraggingActivity> {
    ...
    private ActivityControlHelper.AnimationFactory mAnimationFactory = (t, i) -> { };
    ...
    private int mLogAction = LauncherLogProto.Action.Touch.SWIPE;
    private float mCurrentQuickScrubProgress;
    private boolean mQuickScrubBlocked;

    private @TouchConsumer.InteractionType int mInteractionType = TouchConsumer.INTERACTION_NORMAL;
    ...
    private void executeOnUiThread(Runnable action) {
        if (Looper.myLooper() == mMainThreadHandler.getLooper()) {
            ...
            Utilities.postAsyncCallback(mMainThreadHandler, action);
        }
    }
    ...
    private void setStateOnUiThread(int stateFlag) {
        if (Looper.myLooper() == mMainThreadHandler.getLooper()) {
            ...
            Utilities.postAsyncCallback(mMainThreadHandler, () -> mStateCallback.setState(stateFlag));
        }
    }
    ...
    private boolean onActivityInit(final T activity, Boolean alreadyOnHome) {
        ...
        if (alreadyOnHome) {
            mActivity.clearForceInvisibleFlag(BaseActivity.STATE_HANDLER_INVISIBILITY_FLAGS);
        } else {
            mActivity.addForceInvisibleFlag(BaseActivity.STATE_HANDLER_INVISIBILITY_FLAGS);
        }
    ...
    private void launcherFrameDrawn() {
        MultiValueAlpha.AlphaProperty property = mActivityControlHelper.getAlphaProperty(mActivity);
    ...
    public void updateInteractionType(@TouchConsumer.InteractionType int interactionType) {
        if (mInteractionType != TouchConsumer.INTERACTION_NORMAL) {
            throw new IllegalArgumentException(
                    "Can't change interaction type from " + mInteractionType);
        }
        if (interactionType != TouchConsumer.INTERACTION_QUICK_SCRUB) {
            throw new IllegalArgumentException(
        ...
        animateToProgress(mCurrentShift.value, 1f, QUICK_SCRUB_FROM_APP_START_DURATION, Interpolators.LINEAR,
                true /* goingToHome */);
    }
    ...
    private void updateFinalShift() {
        ...
        if (controller != null) {
            ...
            boolean passedThreshold = shift > 1 - RecentsView.UPDATE_SYSUI_FLAGS_THRESHOLD;
    ...
    private void updateFinalShiftUi() {
        ...
        if (passed != mPassedOverviewThreshold) {
            ...
            if (mInteractionType == TouchConsumer.INTERACTION_NORMAL && mRecentsView != null) {
    ...
    public void onGestureStarted() {
        ...
        setStateOnUiThread(mInteractionType == TouchConsumer.INTERACTION_NORMAL
    ...
    public void onGestureEnded(float endVelocity) {
        ...
        mLogAction = isFling ? LauncherLogProto.Action.Touch.FLING : LauncherLogProto.Action.Touch.SWIPE;
    ...
    private void handleNormalGestureEnd(float endVelocity, boolean isFling) {
        ...
        if (!isFling) {
            ...
            interpolator = goingToHome ? Interpolators.OVERSHOOT_1_2 : Interpolators.DEACCEL;
        } else {
            goingToHome = endVelocity < 0;
            endShift = goingToHome ? 1 : 0;
            startShift = Utilities.boundToRange(currentShift - velocityPxPerMs
                    * Utilities.SINGLE_FRAME_MS / mTransitionDragLength, 0, 1);
    ...
    private void doLogGesture(boolean toLauncher) {
        ...
        if (dp.isVerticalBarLayout()) {
            direction = (dp.isSeascape() ^ toLauncher) ? LauncherLogProto.Action.Direction.LEFT : LauncherLogProto.Action.Direction.RIGHT;
        } else {
            direction = toLauncher ? LauncherLogProto.Action.Direction.UP : LauncherLogProto.Action.Direction.DOWN;
        }

        int dstContainerType = toLauncher ? LauncherLogProto.ContainerType.TASKSWITCHER : LauncherLogProto.ContainerType.APP;
    ...
    public void reset() {
        if (mInteractionType != TouchConsumer.INTERACTION_QUICK_SCRUB) {
    ...
    private void resetStateForAnimationCancel() {
        ...
        mActivity.clearForceInvisibleFlag(BaseActivity.INVISIBLE_BY_STATE_HANDLER);
    }
