...
public class ShelfScrimView extends ScrimView {
    ...
    public ShelfScrimView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mMaxScrimAlpha = Math.round(LauncherState.OVERVIEW.getWorkspaceScrimAlpha(mLauncher) * 255);
    ...
    public void reInitUi() {
        ...
        if (!mDrawingFlatColor) {
            ...
            mMidProgress = LauncherState.OVERVIEW.getVerticalProgress(mLauncher);
    ...
    public void updateColors() {
        ...
        if (mProgress >= 1) {
            ...
            int alpha = Math.round(Utilities.mapToRange(
                    mProgress, mMidProgress, 1, mMidAlpha, 0, Interpolators.ACCEL));
            ...
            int alpha = Math.round(
                    Utilities.mapToRange(mProgress, (float) 0, mMidProgress, (float) mEndAlpha,
                            (float) mMidAlpha, Interpolators.clampToProgress(Interpolators.ACCEL, 0.5f, 1f)));
            mShelfColor = setAlphaComponent(mEndScrim, alpha);

            int remainingScrimAlpha = Math.round(
                    Utilities.mapToRange(mProgress, (float) 0, mMidProgress, mMaxScrimAlpha,
                            (float) 0, Interpolators.LINEAR));
